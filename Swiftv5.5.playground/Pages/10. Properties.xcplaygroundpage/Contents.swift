...
// Stored properties store constant and variable values as part of an instance. (classes and structures)
// Computed properties calculate (rather than store) a value. (classes, structures, and enumerations)
...
struct Calculator {
    static func calculations() -> Int {
        var x: [Int] = []
        for i in 1...4_000 { x.append(i) }
        return x.last!
    }
}
struct Player {
    ...
    lazy var calculationLazy = {
        Calculator.calculations()
    }()
    
    var calculationComputed: Int {
        Calculator.calculations()
    }
    ...
    // We can declare as a computed property instead of lazy but for computed property it computes every time when you access it, but for lazy property it stores it and can be reuse it.
}

var sachin = Player(name: "Sachin Tendulkar", position: "Batsman", team: "India")
print(sachin.calculationLazy) // runs 4000 times
print(sachin.calculationLazy) // used the previous result
print(sachin.calculationComputed) // runs 4000 times
print(sachin.calculationComputed) // runs 4000 times

// Lazy is not thread safe - If a property marked with the lazy modifier is accessed by multiple threads simultaneously and the property has not yet been initialized, there’s no guarantee that the property will be initialized only once.
...
// You have the option to define either or both of these observers on a property:
    // willSet is called just before the value is stored. (has param of newValue and for old value, it is the actual value, called by variable itself)
    // didSet is called immediately after the new value is stored. (has param of oldValue and for new value, it is the actual value, called by variable itself)
class StepCounter {
    var totalSteps: Int = 0 {
        willSet {
            print("About to set totalSteps to \(newValue)") // or without x you can use newValue as default param
        }
        didSet {
...
struct AudioChannel {
    ...
    var currentLevel: Int = 0 {
        didSet {
            if currentLevel > AudioChannel.threshold {
                currentLevel = AudioChannel.threshold
            }
